...
function plot_panel_a(ax, phase, cfg)
    ...
    % --- Region annotations ---
    % Monostable escape (low lambda, high beta)
    %text(ax, 0.10, 0.95, 'Monostable', ...
    %    'Interpreter', 'latex', 'FontSize', 14, ...
    %    'Color', cfg.colors.label_esc, 'FontWeight', 'bold', ...
    %    'BackgroundColor', [1 1 1 0.85], 'Margin', 3);
    %text(ax, 0.10, 0.85, '(tumour escape)', ...
    %    'Interpreter', 'latex', 'FontSize', 12, ...
    %    'Color', cfg.colors.label_esc, ...
    %    'BackgroundColor', [1 1 1 0.85], 'Margin', 2);

    % Bistable region (middle)
    %text(ax, 0.70, 0.60, '\textbf{Bistable}', ...
    %    'Interpreter', 'latex', 'FontSize', 15, ...
    %    'Color', cfg.colors.label_bist, ...
    %    'BackgroundColor', [1 1 1 0.85], 'Margin', 3);

    % Monostable control (high lambda, low beta)
    %text(ax, 1.40, 0.15, 'Monostable', ...
    %    'Interpreter', 'latex', 'FontSize', 14, ...
    %    'Color', cfg.colors.label_ctrl, 'FontWeight', 'bold', ...
    %    'BackgroundColor', [1 1 1 0.85], 'Margin', 3);
    %text(ax, 1.40, 0.06, '(immune control)', ...
    %    'Interpreter', 'latex', 'FontSize', 12, ...
...
function plot_panel_b(ax, bif, cfg)
 ...
 % --- Monostable escape shading (HIGH beta, right of upper fold) ---
    ...
    xline(ax, cfg.ref_beta, ':', ...
        ...
        'LineWidth', 1.5, ...
        'HandleVisibility', 'off');
%     text(ax, cfg.ref_beta + 0.015, 1.42, ...
...
%             'HandleVisibility', 'off');
    ...
    % --- Region annotations ---
    %text(ax, 0.06, 1.35, 'Immune control', ...
     %   'Interpreter', 'latex', 'FontSize', 14, ...
      %  'Color', cfg.colors.label_ctrl, 'FontWeight', 'bold', ...
       % 'BackgroundColor', [1 1 1 0.85], 'Margin', 2);
    %text(ax, 0.06, 1.22, 'control', ...
     %   'Interpreter', 'latex', 'FontSize', 14, ...
      %  'Color', cfg.colors.label_ctrl, 'FontWeight', 'bold', ...
       %'BackgroundColor', [1 1 1 0.85], 'Margin', 2);

%     if any(bist)
%         mid_bist = beta_arr(round(mean(bist_idx)));
%         text(ax, mid_bist, 0.65, '\textbf{Bistable}', ...
%             'Interpreter', 'latex', 'FontSize', 14, ...
%             'Color', cfg.colors.label_bist, ...
...
%     end
   ...
   % text(ax, 1.02, 1.35, 'Tumour escape', ...
    %    'Interpreter', 'latex', 'FontSize', 14, ...
     %   'Color', cfg.colors.label_esc, 'FontWeight', 'bold', ...
      %  'BackgroundColor', [1 1 1 0.85], 'Margin', 2);
   % text(ax, 1.02, 1.22, 'escape', ...
   %     'Interpreter', 'latex', 'FontSize', 14, ...
   %   'Color', cfg.colors.label_esc, 'FontWeight', 'bold', ...
   %    'BackgroundColor', [1 1 1 0.85], 'Margin', 2);
